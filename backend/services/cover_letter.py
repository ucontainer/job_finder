...
from backend.models.schemas import MatchedJob
...
def generate_cover_letter(job: MatchedJob, user_title: str) -> str:
    if not _client:
        return (
            f"I'm excited about the {job.job_title} role at {job.company}. "
            f"With my background as a {user_title}, I believe I can contribute "
            f"meaningfully to your team. I'd love the opportunity to discuss how "
            f"my skills align with your needs."
        )
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    f"- My current/recent title: {user_title}"
                ),
            }
        ],
    )
...
def generate_cover_letters_batch(
    jobs: list[MatchedJob], user_title: str
) -> list[MatchedJob]:
    """Generate cover letters for a list of matched jobs. Mutates and returns the list."""
    for job in jobs:
        job.cover_letter = generate_cover_letter(job, user_title)
    return jobs
